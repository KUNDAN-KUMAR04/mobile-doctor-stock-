...
function renderStockInputs(prefill = {}) {
  ...
  el.innerHTML = places
    .map((p) => {
      const val = prefill[p] !== undefined ? prefill[p] : 0;
      return `<div class="stock-input-row">
        <span class="place-name">${p}</span>
        <input type="number" class="search-input stock-qty-input" data-place="${p}" value="${val}" min="0" />
      </div>`;
    })
    .join("");
}
...
function startEdit(m) {
  ...
  const prefill = {};
  m.stock.forEach((s) => (prefill[s.place] = s.qty));
      ...
      const rows = models
        .map((m) => {
          ...
          return `<div class="model-list-row">
            <span>${m.model} <span style="color: var(--text-muted);">(${m.displayCode || "no code"}) — ${total} in stock</span></span>
            <span class="actions">
