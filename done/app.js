...
const els = {
  ...
  box: document.getElementById("box-select"),
};
...
function populateBoxOptions() {
  const boxes = [...new Set(allModels.flatMap((m) => m.stock.map((s) => s.box).filter(Boolean)))].sort();
  els.box.innerHTML =
    `<option value="">All boxes</option>` +
    boxes.map((b) => `<option value="${b}">${b}</option>`).join("");
}
...
function matchesFilters(m, f) {
  ...
  if (f.box && !m.stock.some((s) => s.box === f.box)) return false;
  if (f.text) {
...
function render() {
  ...
  els.results.innerHTML = matches.map((m) => renderTicket(m, f.box)).join("");
}

function renderTicket(m, activeBox) {
  const total = m.stock.reduce((sum, s) => sum + (Number(s.qty) || 0), 0);
  const stockRows = m.stock
    .map((s) => {
      const qty = Number(s.qty) || 0;
      const cls = qty > 0 ? "in-stock" : "out-stock";
      const boxLabel = s.box ? `<span class="box-tag">${s.box}</span>` : "";
      const highlight = activeBox && s.box === activeBox ? " stock-row-highlight" : "";
      return `<div class="stock-row${highlight}">
...
async function init() {
  ...
  els.box.addEventListener("change", render);
}
